import { SUPPORT_EMAIL } from "./flConfig.js";
...
function summarize(state) {
  ...
  for (let n = 1; n <= TOTAL_DAYS; n++) {
    ...
    if (entry.workout) workoutsTotal += 1;
  }
  ...
  return { daysLogged, fullDays, workoutsTotal, weekWorkouts, weekFull, week };
}
...
function renderHistory() {
  ...
  rows.forEach(({ n, iso, entry, hits }) => {
    ...
    open.addEventListener("click", () => {
      fillEntry(iso);
      window.scrollTo({ top: els.entryDate.offsetTop - 80, behavior: "smooth" });
    });
    li.appendChild(open);
    els.history.appendChild(li);
  });
...
els.saveSetup.addEventListener("click", () => {
  ...
  const calorieTarget = Math.max(800, Number(els.calorieTarget.value) || 2000);
  const stepTarget = Math.max(1000, Number(els.stepTarget.value) || 10000);
  state.startDate = startDate;
  state.calorieTarget = calorieTarget;
  state.stepTarget = stepTarget;
  saveState(state);
...
// Prefer today if inside the 90 days; otherwise Day 1.
(() => {
